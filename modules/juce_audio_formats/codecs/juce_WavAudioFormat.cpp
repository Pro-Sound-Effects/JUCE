...
// we also moved all of the headers and chunk code into AudioFormatFileHelpers.h, so that it is accessible to other classes
// PSE end
    ...
    void doRead (InputStream* in, bool storeChunks)
    {
        ...
        if (input->readInt() == chunkName ("WAVE"))
        {
            ...
            while ((uint64) input->getPosition() < end && ! input->isExhausted())
            {
                ...
                {
                    // PSE start BR: MOD vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
